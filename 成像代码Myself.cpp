...
//声明函数
void rwdata();              //读写数据用子函数
void initialize();          //初始化用子函数
void SOR();                 //SOR算法
void calculate_elec();      //计算电场分布
void calculate_grid();      //计算网格尺寸
void equpotential_line_painting();  //绘制等位线
...
void rwdata()
{
	...
	//向输出文件中写入数据
	fopen_s(&out, "E:\\1120160947\\1120160947.res", "w");       //生成输出文件
	if (out == NULL)//报错
	{
...
void calculate_elec()
{
	...
	fprintf(out, "\t\t迭代次数：%d\t\t迭代因子值:%lf\t\t", Time_total, ω);
	fprintf(out, "平均残差：%lf\t\t最大残差：%.7lf\n", φda, φdm);
...
//绘制等位线
void equpotential_line_painting()
{
